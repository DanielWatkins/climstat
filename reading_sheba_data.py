...
sounding_list = []
for folder in os.listdir(drive_loc):
    if folder[0:2] == '19':
        path = os.path.join(drive_loc, folder)
        print(path)
        for file in os.listdir(path):
            if file[0] is not '.':
                df = read_sheba_data(os.path.join(path, file), min_pressure)
                sounding_list.append(df)
sounding_df = pd.concat(sounding_list)
sounding_df.to_csv(os.path.join(drive_loc, save_prefix + '.csv'))
